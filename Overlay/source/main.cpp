	...
	virtual tsl::elm::Element* createUI() override {
		...
		list->addItem(new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
			if (!SaltySD) {
				...
				if (!(ReverseNX_RT->pluginActive)) renderer->drawString("游戏没有检查任何模式!", false, x, y+40, 18, renderer->a(0xF33F));
				else {
	...
	virtual void update() override {
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				if (!ReverseNX_RT->res.handheld_res) strcpy(HandheldDDR, "掌机模式 DDR: 默认");
				else snprintf(HandheldDDR, sizeof(HandheldDDR), "掌机模式 DDR: %dx%d", resolutions[ReverseNX_RT->res.handheld_res].first, resolutions[ReverseNX_RT->res.handheld_res].second);
				if (!ReverseNX_RT->res.docked_res) strcpy(DockedDDR, "底座模式 DDR: 默认");
				else snprintf(DockedDDR, sizeof(DockedDDR), "底座模式 DDR: %dx%d", resolutions[ReverseNX_RT->res.docked_res].first, resolutions[ReverseNX_RT->res.docked_res].second);
			}
			else i++;
		}
